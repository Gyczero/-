...
class Solution:
    def longestSubsequence(self, arr: List[int], difference: int) -> int:
        ...
        后一个状态计算可以用前一个状态
        动态规划的公式为：
        > 状态存储容器: 哈希，为什么Hash => 因为需要查找前一个值，查找 => hash
        > 当前状态: 动态规划和贪心[i]表示以i为序列最后一个值这个【状态】的最大长度，DP为Hash
        > 前一个状态 动态规划和贪心[当前值 - 差值]

        公式为：动态规划和贪心[i]（当前状态） = 动态规划和贪心[a[i]-difference, 0]（前边一个状态） + 1
